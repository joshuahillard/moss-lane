#!/usr/bin/env python3
"""
Sol-Fortress v3.0 — Unified Clean Rewrite
...
# ══════════════════════════════════════════════════════════════════════════════
os.makedirs("/home/solbot/fortress/logs", exist_ok=True)
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | %(levelname)s | %(message)s",
    handlers=[
        logging.StreamHandler(),
        logging.FileHandler("/home/solbot/fortress/logs/fort_v2.log"),
    ],
)
...
# ══════════════════════════════════════════════════════════════════════════════
class EnvLoader:
    def __init__(self, path: str = "/home/solbot/fortress/.env"):
...
DB_PATH = "/home/solbot/fortress/logs/fortress.db"
        ...
        for wallet in CFG["smart_wallets"]:
            ...
            try:
                url = (f"https://api.helius.xyz/v0/addresses/{wallet}/transactions"
                       f"?api-key=3ef33053-0&limit=5&type=SWAP")
...
async def main():
    log.info("=" * 60)
    log.info("  Sol-Fortress v3.0 — Target: $20,000")
    log.info(f"  Wallet : {WALLET}")
    log.info(f"  Mode   : {'PAPER' if PAPER else 'LIVE'}")
